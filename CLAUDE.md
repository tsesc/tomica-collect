...
- **watch-new-releases.yml** — MANUAL ONLY (`workflow_dispatch`; weekly cron intentionally disabled to avoid double-importing with monthly-scrape). Runs `watch-new` → `enrich-new` → `import-new`: detects new releases via a 5-source diff, does one full Gemini Flash call per car (12 visual attributes + EN/zh-TW names + EN/zh-TW descriptions), INSERTs `source='official'` rows, and commits dated snapshots to `scraper/data/snapshots/`. Requires secrets `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `GEMINI_API_KEY`. ⚠️ After migration 009 `import-new` has NO idempotency key (re-runs can duplicate rows) — keep manual-only; this is an alternative to monthly-scrape's import path, enable its `schedule:` block only after fixing that and retiring monthly-scrape's import.
...
## Known Issues / TODO
- [x] ~~Migrations 006/007 not applied~~ — 006–009 all applied to the live DB (2026-07-12, via Management API). 009 DROPPED 008's uniq_catalog_series_model_official: Tomica reuses regular-series numbers across generations, so (series, model_number) is NOT unique for official rows — the index made ignore-duplicates imports silently drop every number-reuse row
- [ ] `scrape import-new` has no DB-level idempotency key after 009 — re-running it can duplicate rows. Keep watch-new-releases.yml manual-only until it gets a real key (e.g. (series, model_number, car_name))
